> **Nota:** Esta es la versión sin fórmulas LaTeX de [11-oráculos-y-relativización](../../03-computabilidad/11-oráculos-y-relativización.md). Las expresiones matemáticas se muestran en texto plano y Unicode. Para la versión completa con renderizado de fórmulas, consulta el archivo original o el [sitio web del tutorial](https://legalintermediasl.github.io/TeoriaInformacion/).
...
Una **máquina de Turing con oráculo** `M^A` tiene acceso a un conjunto `A ⊆ \{0,1\}^*` como caja negra:
...
- En el siguiente paso, la máquina recibe la respuesta: entra en `q_sí` si `q ∈ A`, o en `q_no` si `q ∉ A`.
...
> **Fórmula:** `P^A = cup_k DTIME^A(n^k)  NP^A = cup_k NTIME^A(n^k)`
...
> **Fórmula:** `P^A = NP^A  y  P^B ≠ NP^B`


### Construcción de A: oráculo que colapsa

Sea `A = PSPACE`. Entonces `P^A = NP^A = PSPACE^A = PSPACE`.
...
> **Fórmula:** `L_B = \{1^n : ∃ x ∈ \{0,1\}^n, x ∈ B\}`


sea difícil para P^B pero fácil para NP^B. El argumento es:

- **NP^B acepta `L_B`:** el testigo es la cadena `x ∈ B` de longitud `n`. Basta verificar la consulta en tiempo constante.
...
El argumento diagonal para el oráculo `B` que separa se entiende mejor con un ejemplo numérico. Fijemos `n = 3`: queremos que `L_B = \{1³\} ∈ NP^B \setminus P^B`.

**Paso 1 — Candidatos de longitud 3.** Hay `2³ = 8` cadenas de longitud 3:
...
**Paso 2 — La MT determinista `M_1` hace consultas en tiempo polinomial.** Supongamos que `M_1`, la primera MT de la enumeración, ejecuta en `n² = 9` pasos sobre entrada `1³` y realiza a lo sumo `⌊ 2^(3/2) ⌋ = 2` consultas (cota de la construcción: `< 2^(n/2)` consultas).

Supón que `M_1` consulta `010` y `101`. Decide que `1³ ∉ L_B` (output 0).

**Paso 3 — Ajuste de `B`.** Las 6 cadenas de longitud 3 no consultadas son: `000`, `001`, `011`, `100`, `110`, `111`. Elegimos **poner `011` en `B`** (cualquiera de las no consultadas sirve). Ahora `∃ x ∈ \{0,1\}³ : x ∈ B` (a saber, `011`), así que `1³` *debería* estar en `L_B`.

Pero `M_1` nunca consultó `011`, de modo que su respuesta (output 0) es **incorrecta**: `M_1` rechaza `1³` aunque `1³ ∈ L_B`. Diagonalizamos: `M_1` falla en la longitud `n = 3`.

**Paso 4 — NP^B acepta trivialmente.** La MT no determinista adivina x = "011" y consulta B: "011" ∈ B → acepta. Un paso de oráculo.

**Paso 5 — Iteración.** Se repite para n = 4, 5, … con la siguiente MT M_2, M_3, … de la enumeración. En cada ronda n, la MT determinista M_n hace < 2^(n/2) consultas, dejando al menos 2^n - 2^(n/2) cadenas de longitud n sin examinar; se pone una en B (o no, según sea necesario para que la MT falle). El conjunto B resultante es un oráculo perfectamente definido.
...
La **algebrización** (Aaronson-Wigderson 2009) extiende el concepto de relativización: en lugar de consultas a `A`, se permite consultar la **extensión algebraica** `Â` de `A` (el polinomio sobre un cuerpo finito que extiende la función característica de `A`). La mayoría de las técnicas conocidas algebrizan, lo que sigue sin ser suficiente para resolver P vs NP.
...
> **Fórmula:** `Σ_0^P = Π_0^P = P`


> **Fórmula:** `Σ_k+1^P = NP^(Σ_k^P)`


> **Fórmula:** `Π_k+1^P = coNP^(Σ_k^P)`


Si P = NP, entonces la jerarquía polinómica colapsa a P. El teorema de Karp-Lipton dice que si NP ⊆ P/poly, la jerarquía colapsa a `Σ_2^P`.
...
> **Fórmula:** `Decidible^K = Σ_2⁰ ∩ Π_2⁰ = Δ_2⁰`


Las MT que deciden con oráculo `K` resuelven exactamente los problemas de nivel `Δ_2⁰` en la jerarquía aritmética. Esto muestra cómo los oráculos "compran" poder computacional en niveles discretos.
...
1. Define formalmente `PSPACE^A` para un oráculo `A`. ¿Por qué `NP^(PSPACE) = PSPACE`?
...
4. Si la jerarquía polinómica colapsara a `Σ_3^P`, ¿qué implicaría eso sobre la relación entre NP y coNP?
