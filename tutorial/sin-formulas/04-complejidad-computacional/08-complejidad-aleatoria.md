> **Nota:** Esta es la versión sin fórmulas LaTeX de [08-complejidad-aleatoria](../../04-complejidad-computacional/08-complejidad-aleatoria.md). Las expresiones matemáticas se muestran en texto plano y Unicode. Para la versión completa con renderizado de fórmulas, consulta el archivo original o el [sitio web del tutorial](https://legalintermediasl.github.io/TeoriaInformacion/).
...
- Si `x ∈ L`: `\Pr[acepta] ≥ 2/3`
- Si `x ∉ L`: `\Pr[acepta] ≤ 1/3`
...
Relación entre las clases: `P ⊆ ZPP = RP ∩ co-RP ⊆ RP ⊆ BPP ⊆ P/poly`.

## Amplificación del éxito

Si un algoritmo en BPP acepta con probabilidad `≥ 2/3` o `≤ 1/3`, repetirlo `k` veces e interpretar el voto mayoritario reduce el error a `2^(-Ω(k))`:


> **Fórmula:** `\Pr[error tras  k  repeticiones] ≤ e^(-2k(1/6)²) = e^(-k/18)`


Con `k = O(log(1/δ))` repeticiones se baja el error a `δ`.

## Ejemplo: test de Miller-Rabin

El **test de primalidad de Miller-Rabin** es un algoritmo en co-RP: dado `n`, elige un testigo aleatorio `a ∈ \{2, …, n-2\}` y comprueba una condición algebraica. Si `n` es primo, siempre acepta. Si `n` es compuesto, rechaza con probabilidad `≥ 3/4`.

El algoritmo se basa en las propiedades de los **pseudo-primos fuertes**. Para `n-1 = 2^s · d` con `d` impar, `n` es primo fuerte en base `a` si:
- `a^d ≡ 1 ±od{n}`, o
- `a^(2^r d) ≡ -1 ±od{n}` para algún `r ∈ \{0,…,s-1\}`.

Si `n` es compuesto, la fracción de bases que lo declaran erróneamente primo es `≤ 1/4`.

Con `k = 40` repeticiones, la probabilidad de error es `≤ 4⁻⁴⁰ ≈ 10⁻²⁴`.
...
Dado un polinomio multivariable `p(x_1,…,x_n)` de grado `d` expresado como árbol de operaciones, ¿es el polinomio cero identicamente?

**Lema de Schwartz-Zippel:** si `p` no es el polinomio cero y se evalúa en un punto aleatorio `r ∈ S^n` donde `S` es un conjunto finito de tamaño `|S| ≥ 2d`, entonces:

> **Fórmula:** `\Pr[p(r_1,…,r_n) = 0] ≤ d/|S|`
...
- **Resultado de Impagliazzo-Wigderson (1997):** si existe un problema en E = `TIME(2^(O(n)))` que requiere circuitos de tamaño `2^(Ω(n))`, entonces BPP = P.
...
- La amplificación permite reducir el error a `2^(-Ω(k))` con `k` repeticiones independientes.
...
1. Muestra que si `L ∈ RP`, entonces existe un algoritmo que, tras `k` repeticiones, rechaza erróneamente con probabilidad `≤ 2^(-k)`.
