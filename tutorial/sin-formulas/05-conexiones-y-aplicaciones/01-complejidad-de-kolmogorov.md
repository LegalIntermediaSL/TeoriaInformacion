> **Nota:** Esta es la versión sin fórmulas LaTeX de [01-complejidad-de-kolmogorov](../../05-conexiones-y-aplicaciones/01-complejidad-de-kolmogorov.md). Las expresiones matemáticas se muestran en texto plano y Unicode. Para la versión completa con renderizado de fórmulas, consulta el archivo original o el [sitio web del tutorial](https://legalintermediasl.github.io/TeoriaInformacion/).
...
|---|---|---|---|
| `\underbrace{00⋯0}_32` | Todo ceros | `print('0'*32)` → `rep(0,32)` | `≈ 10` |
| `\underbrace{01⋯01}_32` | Alternado | `print('01'*16)` → `rep(01,16)` | `≈ 12` |
| `01101011100101000110101110100110` | Sin patrón obvio | Copiar literalmente | `≈ 32` |

**Cota superior para la cadena de ceros:** el programa `print('0'*32)` tiene unos 15 caracteres (≈15 bytes ≈ 120 bits en ASCII). En una máquina universal binaria compacta, basta con algo como el par `(k, n) = (0, 32)` codificado en `O(log n)` bits, así que `K(0³²) ≤ 2log_2 32 + c = 10 + c` para alguna constante `c`.
...
> **Fórmula:** `\Pr_x ~ U\{0,1\^n}[K(x) < n - c] ≤ 2^(-c)`


Para `c = 5`: solo `1/32` de las cadenas de 32 bits tienen `K(x) < 27`. **La cadena típica es casi incompresible.**

**Cálculo para la tercera cadena:** si `01101011...0110` es una muestra aleatoria de `\{0,1\}³²`, esperamos `K(x) ≈ 32` bits (más una constante de descripción de la máquina). El mejor compresor práctico (LZ77, BZIP2) tampoco la reduciría significativamente, lo que es consistente con `K(x) ≈ n`.

**Relación con la entropía:** una fuente Bernoulli(1/2) produce cadenas con entropía `H = 1` bit/símbolo. El valor esperado `𝔼[K(X)]` para `X ~ Bernoulli(1/2)^n` satisface:


> **Fórmula:** `n - O(log n) ≤ 𝔼[K(X)] ≤ n + O(1)`


Para fuentes sesgadas Bernoulli(`p`) con `p ≠ 1/2`, las cadenas típicas tienen `K(x) ≈ nH(p)` bits, exactamente como predice Shannon.
