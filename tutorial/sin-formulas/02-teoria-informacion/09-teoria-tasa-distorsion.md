> **Nota:** Esta es la versión sin fórmulas LaTeX de [09-teoria-tasa-distorsion](../../02-teoria-informacion/09-teoria-tasa-distorsion.md). Las expresiones matemáticas se muestran en texto plano y Unicode. Para la versión completa con renderizado de fórmulas, consulta el archivo original o el [sitio web del tutorial](https://legalintermediasl.github.io/TeoriaInformacion/).
...
Una **función de distorsión** `d: 𝒳 × 𝒳̂ → [0,∞)` mide el coste de reconstruir `x̂` cuando el símbolo original era `x`. Las elecciones más comunes son:

- **Distorsión Hamming:** `d(x,x̂) = 1[x ≠ x̂]` (error por símbolo).
- **Error cuadrático medio (MSE):** `d(x,x̂) = (x-x̂)²`.
- **PSNR en imágenes:** derivado del MSE en escala logarítmica.

La distorsión media de un código es `D = 𝔼[d(X,X̂)]`.

## La función de tasa-distorsión

Dado un nivel de distorsión tolerable `D ≥ 0`, la **función de tasa-distorsión** `R(D)` es la tasa mínima de compresión que permite una distorsión media `≤ D`:


> **Fórmula:** `R(D) = min_p(x̂|x): 𝔼[d(X,X̂)] ≤ D I(X;X̂)`


La minimización es sobre todas las distribuciones condicionales `p(x̂|x)` que inducen una distorsión media `≤ D`. `I(X;X̂)` es la información mutua entre la fuente y su reconstrucción.

**Propiedades de `R(D)`:**
- `R(0) = H(X)` (sin distorsión, la tasa mínima es la entropía).
- `R(D) = 0` para `D ≥ D_max`, donde `D_max` es la distorsión de ignorar la entrada.
- `R(D)` es una función convexa y decreciente en `D`.

## El teorema de codificación con pérdida

**Teorema (Shannon, 1959).** Para una fuente i.i.d. `X^n` y una función de distorsión `d`, existe un código de longitud `nR` bits que permite reconstruir `X̂^n` con distorsión media `≤ D + ε` si y solo si `R > R(D)`.

El resultado tiene dos partes:

- **Alcanzabilidad:** para cualquier `R > R(D)`, existe un código con tasa `R` y distorsión `≤ D`.
- **Cota inferior:** cualquier código con tasa `R < R(D)` necesariamente incurre en distorsión `> D`.

La demostración de alcanzabilidad usa codebooks aleatorios: se eligen `2^(nR)` secuencias `x̂^n` al azar según la distribución marginales óptima `p^*(x̂)`, y se asigna a cada `x^n` el índice del codeword más cercano en distorsión.
...
Sea `X ~ Ber(1/2)` y `d` la distorsión Hamming. Entonces:

> **Fórmula:** `R(D) = 1 - H_b(D),  0 ≤ D ≤ 1/2`


donde `H_b(D) = -Dlog D - (1-D)log(1-D)` es la entropía binaria. Para `D > 1/2`, `R(D) = 0` (se puede ignorar la fuente y adivinar).

**Interpretación:** para reproducir cada bit con tasa de error `≤ D`, se necesitan `1 - H_b(D)` bits por símbolo. Con `D = 0.1` (10% de error), basta con `≈ 0.53` bits; sin error se necesita 1 bit completo.

### Fuente Gaussiana con MSE

Sea `X ~ 𝒩(0, σ²)` y `d(x,x̂) = (x-x̂)²`. Entonces:

> **Fórmula:** `R(D) = 1/2logσ²/D,  0 ≤ D ≤ σ²`


Con `D = σ²` (reconstruir siempre el promedio), `R(D) = 0`. Cada bit extra de tasa cuadruplica la fidelidad (reduce `D` a la mitad cada bit adicional).
...
Hay una dualidad matemática profunda entre la función de tasa-distorsión `R(D)` y la capacidad de canal `C(SNR)`:

- En la tasa-distorsión: se minimiza la información mutua `I(X;X̂)` sujeto a la distorsión.
...
En la práctica, la codificación con pérdida óptima es la **cuantización vectorial**: dado un vector `x^n`, se le asigna el índice del codeword más cercano de un codebook de `2^(nR)` vectores. Con `n → ∞`, la cuantización vectorial puede alcanzar `R(D)`.

La cuantización escalar (símbolo a símbolo) es subóptima en general. La **relajación en cuantización** de Shannon muestra que la cuantización vectorial mejora al escalar en `≤ 0.255` bits.
...
En JPEG:
1. La imagen se divide en bloques de `8 × 8` píxeles.
...
- Para la fuente Bernoulli: `R(D) = 1 - H_b(D)`; para la Gaussiana: `R(D) = 1/2log(σ²/D)`.
...
2. Compara la tasa necesaria para reproducir una fuente Gaussiana `𝒩(0,1)` con distorsión `D = 0.01` y `D = 0.25`.
3. ¿Qué ocurre con `R(D)` cuando `D ≥ D_max`? Interpreta el resultado.
